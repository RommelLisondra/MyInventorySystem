...
const ExpenseList: React.FC<Props> = ({expense,onDelete,onEdit,onReload,onSearch,}) => {
  ...
  const filteredExpense = useMemo(() => {
    if (!searchTerm) return expense;
    return expense.filter((c) =>
      c.expenseNo.toLowerCase().includes(searchTerm.toLowerCase())
    );
  }, [expense, searchTerm]);

  // pagination calculations
  const totalPages = Math.ceil(filteredExpense.length / rowsPerPage);
                ...
                {currentExpense.length > 0 ? (
                  currentExpense.map((c) => (
                    <tr key={c.id}>
                      ...
                      <td>{c.expenseNo}</td>
                      <td>{c.expenseDate instanceof Date ? c.expenseDate.toLocaleString() : c.expenseDate}</td>
                      <td>{c.expenseCategoryId}</td>
                      <td>{c.amount}</td>
                      <td>{c.vendor}</td>
                      <td>{c.referenceNo}</td>
                      <td>{c.notes}</td>
