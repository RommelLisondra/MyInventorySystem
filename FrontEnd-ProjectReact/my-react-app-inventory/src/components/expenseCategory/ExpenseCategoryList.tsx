...
interface Props {
  ...
  onReload: () => void;
  onUpdate: (employee: ExpenseCategory) => void;
  onSearch?: (query: string) => void;
}

const ExpenseCategoryList: React.FC<Props> = ({expenseCategory,onDelete,onEdit,onReload,onSearch,}) => {
  ...
  const filteredExpenseCategory = useMemo(() => {
    if (!searchTerm) return expenseCategory;
    return expenseCategory.filter((c) =>
      c.name.toLowerCase().includes(searchTerm.toLowerCase())
    );
  }, [expenseCategory, searchTerm]);

  // pagination calculations
  const totalPages = Math.ceil(filteredExpenseCategory.length / rowsPerPage);
  const currentExpenseCategory = filteredExpenseCategory.slice(
      ...
      <div className="page-header">
        ...
        <div className="page-btn d-flex gap-2">
          <a
            ...
            onClick={(e) => {
              e.preventDefault();
              navigate(ROUTES.EXPENSE_CATEGORY_CREATE); 
            }}
          >
        ...
        <div className="card-body">
          ...
          <div className="table-responsive">
            <table className="table table-bordered table-striped datanew">
              ...
              <tbody>
                {currentExpenseCategory.length > 0 ? (
                  currentExpenseCategory.map((c) => (
                    <tr key={c.id}>
                      ...
                      <td>{c.description}</td>
                      <td>{c.isActive}</td>
                      <td>{c.createdDate instanceof Date ? c.createdDate.toLocaleString() : c.createdDate}</td>
                      <td>{c.modifiedDate instanceof Date ? c.modifiedDate.toLocaleString() : c.modifiedDate}</td>
                      <td>
                        <button
