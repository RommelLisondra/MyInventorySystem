...
interface Props {
  ...
  onReload: () => void;
  onUpdate: (employee: ItemInventory) => void;
  onSearch?: (query: string) => void;
}

const ItemInventoryList: React.FC<Props> = ({iteminventory,onDelete,onEdit,onReload,onSearch,}) => {
  ...
  const filtereditems = useMemo(() => {
    if (!searchTerm) return iteminventory;
    return iteminventory.filter((c) =>
      c.itemDetailCode.toLowerCase().includes(searchTerm.toLowerCase())
    );
  }, [iteminventory, searchTerm]);
    ...
    <div className="content">
      ...
      <div className="page-header">
        <div className="page-title">
          ...
          <h6>Manage your Item Inventory</h6>
        </div>
        <div className="page-btn d-flex gap-2">
          {/* <a
            ...
            onClick={(e) => {
              e.preventDefault();
              navigate(ROUTES.ITEM_INVENTORY_CREATE); 
            }}
          >
                ...
                {currentitems.length > 0 ? (
                  currentitems.map((c) => (
                    <tr key={c.id}>
                      ...
                      <td>{c.QtyOnOrder}</td>
                      <td>{c.QtyReserved}</td>
                      <td>{c.SalesReturnItemQty}</td>
                      <td>{c.PurchaseReturnItemQty}</td>
                      <td>{c.LastStockCount instanceof Date ? c.LastStockCount.toLocaleString() : c.LastStockCount}</td>
                      <td>{c.recStatus}</td>
                      <td>
                        <button
