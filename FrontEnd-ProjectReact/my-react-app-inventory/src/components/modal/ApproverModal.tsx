...
interface Props {
  show: boolean;
  onClose: () => void;
  onSelect: (employee: EmployeeApprover) => void;   // UPDATED
}

const ApproverModal: React.FC<Props> = ({ show, onClose, onSelect }) => {
    const { employeeApprover, loading, searchEmployeeApprover } = useEmployeeApprover();
    const [keyword, setKeyword] = useState("");

    const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
      const value = e.target.value;
      setKeyword(value);
      searchEmployeeApprover(value);
    };
                ...
                <div className="table-responsive" style={{ maxHeight: "350px" }}>
                  <table className="table table-bordered">
                    ...
                    <tbody>
                      ...
                      {!loading && employeeApprover.length === 0 && (
                        <tr>
                          <td colSpan={3} className="text-center">
                            No records found
                          </td>
                      ...
                      {!loading &&
                        employeeApprover.map((employee: EmployeeApprover) => (
                          <tr key={employee.id}>
                            <td>
