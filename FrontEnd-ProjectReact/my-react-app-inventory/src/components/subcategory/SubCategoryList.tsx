...
interface Props {
  ...
  onReload: () => void;
  onUpdate: (employee: SubCategory) => void;
  onSearch?: (query: string) => void;
}

const ApproverList: React.FC<Props> = ({subCategory,onDelete,onEdit,onReload,onSearch,}) => {
  ...
  const filteredSubCategory = useMemo(() => {
    if (!searchTerm) return subCategory;
    return subCategory.filter((c) =>
      c.subCategoryName.toLowerCase().includes(searchTerm.toLowerCase())
    );
  }, [subCategory, searchTerm]);

  // pagination calculations
  const totalPages = Math.ceil(filteredSubCategory.length / rowsPerPage);
  const currentSubCategory = filteredSubCategory.slice(
      ...
      <div className="page-header">
        ...
        <div className="page-btn d-flex gap-2">
          <a
            ...
            onClick={(e) => {
              e.preventDefault();
              navigate(ROUTES.SUB_CATEGORY_CREATE); 
            }}
          >
