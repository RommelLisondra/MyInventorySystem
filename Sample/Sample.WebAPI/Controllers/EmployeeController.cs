...
namespace Sample.WebAPI.Controllers
{
    ...
    public class EmployeeController : ControllerBase
    {
        ...
        public EmployeeController(IServiceManager serviceManager, IMapper mapper)
        {
        ...
        [HttpGet("paged")]
        [ProducesResponseType(typeof(PagedResponse<IEnumerable<EmployeeModel>>), (int)HttpStatusCode.OK)]
        [ProducesResponseType((int)HttpStatusCode.InternalServerError)]
        public async Task<IActionResult> GetAllPaged(int pageNumber = 1, int pageSize = 20)
        {
            try
            {
                var employees = await _serviceManager.EmployeeService.GetAllPaged(pageNumber, pageSize);
                return Ok(employees);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> Create([FromBody] EmployeeModel employeeModel)
        {
            ...
            try
            {
                ...
                await _serviceManager.EmployeeService.CreateEmployee(employeeDto, createdBy);

                var returnedId = employeeDto?.id ?? 0;

                var createdModel = _mapper.Map<EmployeeModel>(employeeDto);
        ...
        {
            if (employeeModel == null || id != employeeModel.id)
            ...
            try
            {
                ...
                await _serviceManager.EmployeeService.UpdateEmployee(employeeDto, editedBy);
        ...
        public async Task<IActionResult> Search([FromQuery] string? keyword)
        {
            try
            {
                var employees = await _serviceManager.EmployeeService.SearchEmployeeByKeywordAsync(keyword);
                return Ok(employees);
            }
            catch (Exception ex)
            {
